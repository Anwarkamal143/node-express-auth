...
import { HTTPSTATUS, HttpStatusCode } from '@/config/http.config';
import { ErrorCode, mapStripeErrorToCode, StripeErrorType } from '@/enums/error-code.enum';
import Stripe from 'stripe';

type ErrorCategory = 'operational' | 'programming' | 'security' | 'infrastructure';
...
export default class AppError extends Error {
  ...
  public static createValidationError(
    ...
    return new AppError(message, HTTPSTATUS.UNPROCESSABLE_ENTITY, ErrorCode.VALIDATION_ERROR, {
      metadata: {
        context,
...
function getHttpStatusForStripeError(error: Stripe.StripeRawError): HttpStatusCode {
