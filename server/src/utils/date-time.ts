...
export function formatRelativeTo(date: Date, baseDate: Date = new Date()): string {
  return formatRelative(date, baseDate);
}

// Custom date parsing with format
// Example: parseDateFromString("2024-10-30", "yyyy-MM-dd") -> Date object for 2024-10-30
export function parseDateFromString(dateString: string, dateFormat: string = 'yyyy-MM-dd'): Date {
...
export function getFiscalQuarter(date: Date, fiscalYearStartMonth: number = 10): number {
  const fiscalQuarter = Math.floor(((getMonth(date) + (12 - fiscalYearStartMonth)) % 12) / 3) + 1;
